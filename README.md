...
I built an infrastructure telemetry pipeline that collects CPU, memory, disk, and system-load metrics from a Linux machine and makes that data available for persistent storage and visualization. I wrote a Python collector that gathers the host-level information from Linux, and separated the collection and transformation responsibilities so that metrics.py normalizes the raw system information into a consistent metrics schema, while database.py handles persistence. I intentionally implemented three storage backends—SQLite as a local embedded database and PostgreSQL and MySQL as network-accessible database services—to understand the difference between local and client/server persistence. I used Ansible for host-level configuration management and deployment: the playbook creates the application directories and Python virtual environment, installs the required runtime dependencies, deploys the collector and configuration files, initializes the SQLite schema, and installs the systemd service so the collector can run as a managed Linux service and restart automatically. I used Docker Compose to orchestrate the supporting infrastructure, including PostgreSQL, MySQL, Prometheus, and Grafana, creating a common Docker network so the containers could communicate using service names rather than hard-coded IP addresses. For observability, I configured Prometheus with a scrape target and investigated its target health, which taught me that configuring a Prometheus target is separate from actually exposing a /metrics endpoint. The Grafana dashboards I completed were backed by PostgreSQL, where Grafana executed SQL queries against the system_metrics table and transformed the results into time-series visualizations for CPU, memory, disk usage, and other host metrics. I verified the pipeline end-to-end by checking that telemetry was being collected and that rows were actually being inserted into SQLite, PostgreSQL, and MySQL.

How were local databases different than network accessible databases in this project?

The key thing I learned is that a local database and a network-accessible database have different infrastructure requirements even when the application code is doing the same logical operation. With SQLite, the collector simply opens a local file such as /opt/infra-telemetry/data/telemetry.db, so there is no database server, network connection, authentication, port, or service discovery involved. PostgreSQL and MySQL are different: they run as separate database services in Docker containers, so the collector has to connect over the network using a host, port, database name, username, and password. In this project, Docker Compose created the database containers and their shared network, while database.py acted as the client that connected to those services and inserted the telemetry. This taught me that moving from local storage to a network-accessible database introduces additional infrastructure concerns—service availability, networking, DNS/service names, ports, credentials, connection failures, and database initialization—that don't exist when you're just writing to a local SQLite file.
